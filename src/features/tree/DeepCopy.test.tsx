...
My Notes
  ...
  Target
    ...
    const bobNodeEvents = bob()
      ...
      .filter(
        (e: Event) =>
          e.kind === KIND_KNOWLEDGE_DOCUMENT && e.pubkey === BOB.publicKey
      );
    const bobNodeDTags = bobNodeEvents.flatMap((event: Event) => {
...
My Notes
  ...
  test("Forking readonly root publishes a snapshot event and stores its pointer", async () => {
    ...
    expect(
      bobDocumentEvents.some((event: Event) =>
        event.content.includes('snapshot="')
      )
    ).toBe(true);
  });
});
