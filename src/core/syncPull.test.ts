...
test("pullSyncWorkspace uses configured relays only and writes raw markdown documents into DOCUMENTS", async () => {
  ...
  expect(fs.readFileSync(path.join(tempDir, "AGENTS.md"), "utf8")).toContain(
    "generated by `knowstr sync pull`"
  );
});
...
test("pullSyncWorkspace refetches documents from a 7 day buffer", async () => {
  ...
  expect(aliceDocQuery?.filter.since).toBe(FIRST_ALICE_CREATED_AT - LOOKBACK_SECONDS);
  expect(bobDocQuery?.filter.since).toBe(FIRST_BOB_CREATED_AT - LOOKBACK_SECONDS);
  expect(manifest.authors).toEqual([
    { pubkey: ALICE, last_document_created_at: FIRST_ALICE_CREATED_AT },
    {
      pubkey: BOB,
      last_document_created_at: FIRST_BOB_CREATED_AT + LOOKBACK_SECONDS + ONE_DAY_SECONDS,
    },
  ]);
