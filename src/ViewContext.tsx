...
export function getContext(
  ...
  if (parentRelation) {
    return parentContext.push(getRelationNodeID(parentRelation));
  }
...
function relationMatchesRequestedNode(
  ...
  return shortID(getRelationNodeID(relation)) === localID;
}
...
function getAuthorCandidateRelations(
  ...
  return getIndexedRelationsForKeys(authorDB, [localID]);
}
...
function resolveRequestedStackFromRoot(
  ...
  const actualStack: ID[] = [getRelationNodeID(rootRelation)];
  ...
  for (let index = 1; index < requestedStack.length; index += 1) {
    ...
    actualStack.push(getRelationNodeID(nextRelation));
...
export function getDescendantRelations(
  ...
  return allRelations.filter(
    (relations) =>
      ...
      ((shortID(getRelationNodeID(relations)) === localID &&
...
export function getDisplayTextForView(
  ...
  const [nodeID] = getNodeIDFromView(data, viewPath);
  if (isSearchId(nodeID as ID)) {
    const query = parseSearchId(nodeID as ID) || "";
    return `Search: ${query}`;
  }
...
export function newRelations(
  ...
  const localNodeID = shortID(nodeID) as ID;
  let relationText = text ?? "";
  if (text === undefined) {
    if (localNodeID === LOG_NODE_ID) {
      relationText = "~Log";
    } else if (localNodeID === EMPTY_NODE_ID) {
      relationText = "";
    } else if (isSearchId(localNodeID)) {
      relationText = parseSearchId(localNodeID) || "";
    }
  }
  const relationTextHash =
    isSearchId(localNodeID)
      ? localNodeID
      : text !== undefined || localNodeID === LOG_NODE_ID || localNodeID === EMPTY_NODE_ID
