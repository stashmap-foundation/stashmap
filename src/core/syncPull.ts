...
- \`knowstr write remove-item --parent <relation-id> --item <item-id>\`: remove an item from a parent; owned subtrees are deleted
...
- use relation-aware commands such as \`set-text\`, \`create-under\`, \`link\`, \`set-relevance\`, \`set-argument\`, \`move-item\`, or \`remove-item\`
- rerun \`knowstr sync pull\`
