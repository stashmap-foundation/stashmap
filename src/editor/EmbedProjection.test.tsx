...
test("rewording a moved row keeps its place and the bond", async () => {
  const workspacePath = writeWorkspace({
    "note.md": [
      "# Note <!-- id:note -->",
      "",
  ...
  await userEvent.clear(editor);
  await userEvent.type(editor, "La Pedrera{Escape}");
...
Note
  ...
  await waitFor(() => {
    ...
    expect(note).toMatch(
      /- La Pedrera ~~\[Casa Mila\]\(#cm\)~~ <!-- id:\S+ embed="true" front="true" -->/u
    );
    expect(note.match(/front="true"/gu)).toHaveLength(1);
  });

  fireEvent.dragStart(screen.getByRole("treeitem", { name: "La Pedrera" }));
  fireEvent.drop(screen.getByRole("treeitem", { name: "Casa Battlo" }));
...
Note
  ...
  await waitFor(() => {
    ...
    expect(note).toMatch(
      /- La Pedrera ~~\[Casa Mila\]\(#cm\)~~ <!-- id:\S+ embed="true" after="cb" -->/u
    );
    expect(note.match(/La Pedrera/gu)).toHaveLength(1);
    expect(note).not.toMatch(/front="true"/u);
  });
});

test("a multi-row drop chains the anchors", async () => {
  const workspacePath = writeWorkspace({
    "note.md": [
      "# Note <!-- id:note -->",
      "",
  ...
  await clickRow("Casa Mila");
  await userEvent.keyboard("{Shift>}j{/Shift}");

  fireEvent.dragStart(screen.getByRole("treeitem", { name: "Casa Mila" }));
  fireEvent.drop(screen.getByRole("treeitem", { name: "Park Guell" }));
...
Note
  ...
  await waitFor(() => {
    ...
    expect(note).toMatch(
      /- \[Casa Mila\]\(#cm\) <!-- id:\S+ embed="true" after="pg" -->/u
    );
    expect(note).toMatch(
      /- \[Casa Battlo\]\(#cb\) <!-- id:\S+ embed="true" after="cm" -->/u
    );
...
test("a depth drop into a projected row nests through its placement", async () => {
  const workspacePath = writeWorkspace({
    "note.md": [
      "# Note <!-- id:note -->",
      "",
  ...
  fireEvent.dragStart(screen.getByRole("treeitem", { name: "Plain" }));
  setDropIndentLevel("Plain", "Casa Battlo", 4);
  fireEvent.drop(screen.getByRole("treeitem", { name: "Casa Battlo" }));
...
Note
  ...
  await waitFor(() => {
    ...
    expect(note).toMatch(
      /- \[Casa Battlo\]\(#cb\) <!-- id:\S+ embed="true" -->\n {4}- Plain <!-- id:plain front="true" -->/u
    );
    expect(note).not.toMatch(/^- Plain/mu);
  });
});
