...
function usage(): string {
  return [
    ...
    "",
    "The loop ignores idea.md, implementation.md, and .idea/**, verifies npm run test/lint/typescript after each pass, instructs child reviews to enforce applicable markdown AC/hard greps, reject complexity/performance regressions and DB scans, and creates descriptive commits for successful code changes.",
  ].join("\n");
}
...
function buildReviewPrompt(input: {
  ...
  return [
    ...
    "- Review code/configuration plus every project instruction, implementation-plan, acceptance-criteria, migration prompt, or hard-gate document that applies to the changed code. Markdown specs are mandatory review input, not background prose.",
    "- AGENTS.md is a reviewable project instruction file; enforce it when it changes.",
    "- If the branch adds, changes, or appears to implement a markdown spec such as row-model-plan.md, performance-regression.md, tasks/*.md, any *plan*.md, or any migration/review prompt, read it and treat its acceptance criteria, forbidden patterns, non-negotiable rules, and hard grep gates as mandatory.",
    ...
    "",
    "Core review intent:",
    "- This is an acceptance-criteria conformance and complexity-reduction pass, not a broad style pass.",
    "- First extract the required end state from the applicable markdown: goals, acceptance criteria, allowed inputs, forbidden inputs, prohibited compatibility moves, verification commands, and hard greps.",
    "- Then compare the changed implementation to that checklist. Passing tests do not make code acceptable when it violates the markdown.",
    "- When an AC or documented rule is violated, fix the implementation so it satisfies the documented end state. Do not merely report the violation or paper over it with compatibility behavior.",
    "- Do not preserve the old model under new names. Prefer direct data flow from the correct source of truth over compatibility wrappers, fallback paths, caches, registries, or parallel projections.",
    "",
    "Non-negotiable review checks:",
    "- Do not report CLEAN if AGENTS.md, the user request, or any applicable markdown acceptance criterion is violated.",
    "- Do not stop at finding an AC violation. Fix it properly by following the applicable markdown's allowed data flow, forbidden patterns, verification steps, and hard gates.",
    "- Run or adapt markdown-specified grep/hard-gate commands when applicable; inspect every remaining hit and reject hits outside the documented allowed cases.",
    "- Ask what can be deleted while preserving behavior. Prefer merging, deleting, or simplifying over layering another abstraction.",
    "- Reject avoidable optionality, casts, compatibility wrappers, duplicated mapping/parsing/validation, broad helpers, OOP-style manager/service abstractions, and dead or speculative code.",
    "- Check bugs, edge cases, broken error handling, race/state issues, incorrect assumptions, unnecessary repeated work, and avoidable expensive operations.",
    "",
    "Performance hard gates:",
    "- No DB scans. Hard-fail new or preserved all-DB, all-node, all-source, or whole-collection scans used as identity recovery, lookup fallback, validation shortcut, or hot-path implementation.",
    "- No performance degradation. Check rendering, traversal, DnD/actions/planner paths, import/save flows, and validation paths for added repeated work, eager computation, broad invalidation, extra subscriptions, sync loops, or O(n) work inside existing per-row/per-node loops.",
    "- Do not replace explicit row/ref/index/state inputs with Object.values(...).find, database traversal, global map scans, parsed UI keys, or similar fallback lookups.",
    "- If a change removes an index/cache/lazy boundary or moves work into a hotter path, compare the before/after call path and do not report CLEAN without clear evidence that the cost did not regress.",
    "",
    "Row-model and migration hard fails when applicable:",
    "- No compatibility side channels or optional backup identity paths: no useOptionalRow, no hooks that use RowContext when present but fall back to getNodeForView/getCurrentEdgeForView/getRowIDFromView, no VirtualRowsMap/provider/hooks, no viewKey-keyed identity maps, and no all-DB fallback scans.",
    "- Planner/core mutation code must receive explicit graph/edit inputs, not Row, ViewPath, viewKey, parsed UI keys, or fallback lookups, unless the applicable markdown explicitly allows a narrow view-state use.",
    "",
    "Hard fail handling:",
    "- If the branch violates AGENTS.md, the user request, any applicable markdown AC, a hard grep gate, the no-DB-scan rule, or the no-performance-regression rule, do not report CLEAN.",
    "- Fix the problem directly when safe, especially AC violations: change the code to match the documented design instead of adding shims, fallbacks, or exceptions. If it cannot be fixed safely in this pass, explain why and report BLOCKED.",
    "",
    ...
    "2. Read AGENTS.md, inspect the relevant diff, and read the applicable markdown specs before judging the code.",
    "3. Build a checklist from the markdown AC, non-negotiable rules, prohibited patterns, and hard grep gates; compare the implementation to that checklist line by line.",
    "4. Run applicable hard greps/searches, inspect surrounding code for remaining hits, and reject compatibility side channels or old-model behavior hidden under new names.",
    "5. Perform the complexity/performance review: delete or simplify wrappers/fallbacks/duplication where safe, and block DB scans or performance regressions.",
    "6. If you find actionable issues or AC violations, fix them directly using the applicable documents as the source of truth. For bugs, follow the bug reproduction workflow before changing production code.",
    "7. Run all mandatory validation commands listed above; tests must not use --runInBand.",
    "8. If no actionable issue remains and validation passes, leave files unchanged.",
    "9. If an issue or validation failure is real but cannot be safely fixed in this pass, explain why and mark BLOCKED.",
    "",
    ...
    "- For CLEAN, use REVIEW_LOOP_SUMMARY to say that no actionable issue remained; explicitly mention that AGENTS.md, applicable markdown AC/hard greps, no-DB-scan, no-performance-regression, and validation gates found no blockers. Omit REVIEW_LOOP_FIXES or write - none.",
    "- For BLOCKED, use REVIEW_LOOP_SUMMARY plus a bullet explaining the blocker and the safest next step.",
    "",
    "Final status contract:",
    "- Use REVIEW_LOOP_STATUS: CLEAN only if you found no actionable issue in scope, all applicable markdown AC/hard greps are satisfied, no DB scan or performance regression remains, made no file changes in this pass, and npm run test, npm run lint, and npm run typescript all pass.",
    "- Use REVIEW_LOOP_STATUS: FIXED if you fixed at least one issue, AC violation, hard-gate violation, or validation failure and all three mandatory validation commands pass.",
