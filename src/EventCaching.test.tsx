...
test(
  "published events are cached and available on reload",
  async () => {
...
My Notes
  ...
  France
    ...
    const freshAlice = () => ({ ...alice(), relayPool: mockRelayPool() });
...
test(
  "status bar shows pending when outbox has events on reload",
  async () => {
    const [alice] = setup([ALICE]);
...
test(
  "relay results appear after queue flushes pending outbox events on reload",
  async () => {
    const [alice] = setup([ALICE]);
